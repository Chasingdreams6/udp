A1: 网络数据包处理对于内存的访问很频繁，使用大页的目的是为了降低TLB miss，减少访存开销并减少内存碎片。传统的操作系统将物理内存分配成小的页面，通常大小为4KB。但是对于高速网络数据包处理和存储，4KB页面可能会导致大量的内存碎片，从而影响性能。DPDK使用2MB or 1GB的大页，可以减少页表项的数目，减少TLB的数目，也就减少了TLB miss的次数。
